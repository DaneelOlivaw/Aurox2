...
#	end
	...
	if nomemac.text==("") or idfiscmac.text==("") or bollomac.text==("") or comboreg.active == -1
		...
		if Macellis.cerca(nomemac.text.upcase, idfiscmac.text.upcase, bollomac.text.upcase, comboreg.active_iter[0]) == nil
	...
	#			selmacdest = Macellis.find(:all)
				Macellis.tutti.each do |macdest|
...
#					itermacdest[1] = macdest["nomemac"]
#					itermacdest[2] = macdest["bollomac"]
#					itermacdest[3] = macdest["region"]
#				end
			end
			Conferma.conferma(minsmacelli, "Macello inserito.")
			minsmacelli.destroy
		else
			Errore.avviso(minsmacelli, "Il macello è già presente.")
		end
	end
	}
