...
def moddetentori
	...
	def caricadet
		...
		Detentoris.seleziona.each do |p|
...
#		tipoif1.active=("#{@combodet.active_iter[4]}")
	...
	inseriscidet = Gtk::Button.new( "Modifica detentore" )
	inseriscidet.signal_connect("clicked") {
	...
	boxmoddetentori5.pack_start(inseriscidet, false, false, 0)

	bottelimina = Gtk::Button.new( "Elimina" )
	bottelimina.signal_connect("clicked") {
		#puts Relazs.count(:conditions => ["detentori_id = ?", combodet.active_iter[0]])
		if Relazs.count(:conditions => ["detentori_id = ?", combodet.active_iter[0]]) == 0
			avviso = Gtk::MessageDialog.new(mmoddetentori, Gtk::Dialog::DESTROY_WITH_PARENT, Gtk::MessageDialog::QUESTION, Gtk::MessageDialog::BUTTONS_YES_NO, "Proseguo con l'eleminazione del detentore?")
			risposta = avviso.run
			avviso.destroy
			if risposta == Gtk::Dialog::RESPONSE_YES
				Detentoris.delete(combodet.active_iter[0])
				Conferma.conferma(mmoddetentori, "Detentore eliminato.")
				nomedet.text=("")
				idfisc.text=("")
				caricadet
				combodet.model=(@listadet)
			else
				Conferma.conferma(mmoddetentori, "Operazione annullata.")
			end
		else
			Conferma.conferma(mmoddetentori, "Il detentore non può essere eliminato perché in uso.")
		end
	}
	boxmoddetentori5.pack_start(bottelimina, false, false, 0)
