...
#		end
	Ragsocs.seleziona.each do |rsoc|
		iterragsoc = listaragsoc.append
		iterragsoc[0] = rsoc.ragsoc
		iterragsoc[1] = rsoc.id
		if rsoc.idf == "I"
	...
	Detentoris.seleziona.each do |d|
...
#			if cont.length == 0
			...
			if Relazs.controllo(combo317.active_iter[1], comboragsoc.active_iter[1], combodet.active_iter[1], comboprop.active_iter[1], tipoallev) == nil
				#puts "Niente"
...
#				puts allev.length
				...
				if Allevamentis.cerca(comboragsoc.active_iter[0], comboragsoc.active_iter[2], combo317.active_iter[0]) == nil
