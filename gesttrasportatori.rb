...
def instrasportatori(listatrasp)
	...
	inseriscitrasp.signal_connect("clicked") {
		if nometrasp.text == ""
			...
			if Trasportatoris.cerca(nometrasp.text.upcase) == nil
				...
				Conferma.conferma(minstrasportatori, "Trasportatore inserito.")
				minstrasportatori.destroy
			else
				Errore.avviso(minstrasportatori, "Il trasportatore è già presente.")
			end
		end
	}
