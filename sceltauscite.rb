...
#			itermov[14] = m.marca_padre
	...
	def elenca(lista, cerca)
		lista.clear
		#selmov = Animals.find(:all, :from => "animals", :conditions => ["relaz_id= ? and uscito = ? and marca LIKE ?", "#{@stallaoper.id}", "0", "%#{cerca}%"])
		Animals.capi(@stallaoper.id, cerca).each do |m|
			itermov = lista.append
			itermov[0] = m.id.to_i
#			itermov[1] = m.tipo
			itermov[1] = m.marca
#			itermov[3] = m.specie
#			itermov[4] = m.razza.id
			itermov[2] = m.data_nas #.strftime("%d%m%Y")
#			itermov[6] = m.stalla_nas
#			itermov[7] = m.sesso
#			itermov[8] = m.naz_orig
#			itermov[9] = m.naz_nasprimimp
