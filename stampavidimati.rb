...
def mascvidimati
	...
	stampavidim.signal_connect("clicked") {
		if npagine.text == "" or nultimo.text == ""
			...
			registro = Prawn::Document.new(:page_size => "A4", :page_layout => orientation, :top_margin => 10.mm, :left_margin => 10.mm, :right_margin => 10.mm, :bottom_margin => 10.mm, :compress => true, :info => {:Title => "Registro vidimato", :Author => "Aurox",:Creator => "Aurox", :Producer => "Prawn", :CreationDate => Time.now}) #.generate "altro/prova2.pdf" do
			...
			registro.repeat :all do
				...
				registro.text "REGISTRO AZIENDALE DI #{testotiporeg} BOVINI - PAG.                       - STALLA #{@stallaoper.stalle.cod317}", :align => :center
				registro.text "DI #{@stallaoper.ragsoc.ragsoc}", :align => :center
				registro.stroke do
			...
			options = {:at => [inizio, riga], :width => 100, :align => :left, :start_count_at => prpagina} # :size => 9}
			string = "<page>/#{prog[1]}"
			registro.number_pages string, options
			#registro.number_pages string, options
