...
#	puts arr2.inspect
	...
	arrdoc.each do |m|
		#puts m.inspect
		sel = Animals.find(:all, :select => "uscita", :from => "animals", :conditions => ["relaz_id= ? and #{tipodocumento} LIKE ?", "#{@stallaoper.id}", "#{m}"])
...
#selcapo, marca, nascita, nazorig, naznas, madre, movingr, dataingr, nazprov
		...
		selcapi = Animals.find(:all, :from => "animals", :conditions => ["relaz_id= ? and #{tipodocumento} LIKE ?", "#{@stallaoper.id}", "#{selmov.selected[0]}"])
		#puts selcapi.length
