...
def moduscita(selcapo)
	...
	contamovusc = -1
	while combomovusc.active_iter[1] != capomod[28]
		contamovusc+=1
	...
	if capomod[40].to_s != ""
		...
		contatrasp = -1
		while combotrasp.active_iter[1] != capomod[40]
			contatrasp += 1
	...
	boxmodc45.pack_end(labelsost, false, false, 5)
	sost = Gtk::Entry.new()
	sost.max_length=(14)
	sost.text = ("#{capomod[41]}")
	boxmodc46.pack_start(sost, false, false, 5)
	...
	bottmodcapousc.signal_connect("clicked") {
		...
		Animals.update(capomod[0], {:uscite_id => "#{combomovusc.active_iter[0]}", :uscita => "#{datauscingl.to_i}", :nazdest_id => "#{nazdest}", :certsanusc => "#{certsan.text.upcase}", :data_certsanusc => "#{datacertsaningl.to_i}", :allevusc_id => "#{alldest}", :macelli_id => "#{macdest}", :mod4usc => "#{mod4.text.upcase}", :data_mod4usc => "#{datamod4ingl.to_i}", :trasportatori_id => "#{combotrasp.active_iter[0]}"})
...
# Bottone di eliminazione del movimento
	...
	bottelimina.signal_connect("clicked") {
		...
		if risposta == Gtk::Dialog::RESPONSE_YES
			Animals.update(capomod[0], {:uscite_id => "", :uscita => "", :nazdest_id => "", :certsanusc => "", :data_certsanusc => "", :allevusc_id => "", :macelli_id => "", :mod4usc => "", :data_mod4usc => "", :trasp => "", :uscito => "0"})
