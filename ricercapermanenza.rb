...
#	vistaperm.set_enable_grid_lines(true)
	trova210.signal_connect("clicked") {
		...
		if valtipo == "presenti"
			...
			selperm = Animals.find(:all, :conditions => ["relaz_id= ? and uscito IS NULL and data_ingr <= ?", "#{@stallaoper.id}", "#{giorno.to_date}"])
		else
			selperm = Animals.find(:all, :select => "*, DATEDIFF(uscita,data_ingr) as perman", :conditions => ["relaz_id= ? and YEAR(uscita) = ? and DATEDIFF(uscita,data_ingr) >= ?", "#{@stallaoper.id}", "#{comboanno.active_iter[0]}", "#{giornipres}"]) #, :order => "perman DESC")
			#selperm.each do |s|
...
#		puts giornolibero1
		if valtipo == "presenti"
			...
			selperm = Animals.find(:all, :conditions => ["relaz_id= ? and uscito IS NULL and data_ingr <= ?", "#{@stallaoper.id}", "#{giornolibero.strftime("%Y-%m-%d")}"])
