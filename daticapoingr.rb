def inscapo(progr, anno)
	...
	@arraypres = Array.new
	if Animals.presenti2(@stallaoper.id).length != 0
		Animals.presenti2(@stallaoper.id).each do |pres|
	...
	@stallanas.signal_connect("changed") {
		if primoins == 0
			begin
				...
				elsif @marca.text[0,2].upcase != "IT" and @stallanas.text.upcase == @stallaoper.stalle.cod317 #prima importazione
					@comboing.set_active(0)
					x = -1
	...
	bottinscons.signal_connect("clicked") {
		...
		if @primocapo == 1
			...
			@combonazorig.set_active(0)
			contanazorig = -1
			while @combonazorig.active_iter[0] != @depositoingr["nazorig"]
				contanazorig+=1
				@combonazorig.set_active(contanazorig)
			end
			@combonaznas.set_active(0)
			contanaznas = -1
			while @combonaznas.active_iter[0] != @depositoingr["naznasprimimp"]
				contanaznas+=1
		...
		if @primocapo == 1
			...
			bottprossimo2.signal_connect("clicked") {
				begin
					...
					if @marca.text == "" or @madre.text == "" or @razzaid == 0 or @nascita.text == ""
						Errore.avviso(finestraingr, "Mancano dei dati obbligatori.")
						errore = 1
					elsif @marca.text[0,2].upcase == "IT" and @marca.text.length < 14
						Errore.avviso(finestraingr, "Marca corta.")
