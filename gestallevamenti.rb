...
def mascallevam(listall)
	...
	inserisciall.signal_connect("clicked") {
		...
		if nomeallev.text==("") or idfisc.text==("") or cod317.text==("")
			...
			#puts Allevamentis.cerca(nomeallev.text.upcase, idfisc.text.upcase, cod317.text.upcase).inspect
			if Allevamentis.cerca(nomeallev.text.upcase, idfisc.text.upcase, cod317.text.upcase) == nil
				...
				if listall != nil
					...
					Allevamentis.tutti.each do |all|
...
#					end
				...
				Conferma.conferma(mallevam, "Allevamento inserito.")
				mallevam.destroy
			else
				#puts "già presente"
				Errore.avviso(mallevam, "L'allevamento è già presente.")
			end
		end
	}
